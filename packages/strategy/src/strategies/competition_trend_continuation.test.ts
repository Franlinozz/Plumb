...
const regime: RegimeAssessment = Object.freeze({ label: 'trending_up', confidence: 0.6,
  timeframe: '1H', inputs: Object.freeze({}), reasons: Object.freeze(['test']) });

function risingSeries(): readonly Candle[] {
  const rows: Candle[] = [];
  for (let index = 0; index < 300; index += 1) {
    const close = 100 + index * 0.025 + Math.sin(index / 2.3) * 0.32;
    rows.push({ ts: index * HOUR, open: close - 0.03, high: close + 0.12, low: close - 0.12,
      close, volume: 100, volumeCcy: 100, volumeQuote: 10_000, closed: true });
  }
  const prior = rows[298] as Candle;
  rows[299] = { ...(rows[299] as Candle), open: prior.close,
    close: prior.close + 0.22, high: prior.close + 0.30, low: prior.close - 0.08, volume: 100 };
  return rows;
}

describe('competition trend continuation v4', () => {
  it('is isolated from the P8 strategy registry', () => {
    expect(ALL_STRATEGIES.some((strategy) => strategy.id === COMPETITION_TREND_CONTINUATION_ID)).toBe(false);
  });

  it('emits a symmetric, risk-defined continuation draft on closed data', () => {
    ...
    const config = { ...DEFAULT_STRATEGY_CONFIG, takeProfitR: Object.freeze([2]),
      maxHoldBars: 24, expiryBars: 1 };
    const drafts = competitionTrendContinuation.evaluate({ snapshot, regime, state: EMPTY_STATE,
      config, now, peers: [] });
